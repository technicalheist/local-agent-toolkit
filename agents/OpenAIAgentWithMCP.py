...
# Import default tools and callables
try:
    from helper.tools_definition import tools as default_tools
    from helper.tools import (
        list_files, read_file, write_file, list_files_by_pattern,
        ask_any_question_internet, mkdir, execute_shell_command
    )
    
    # Default tool callables
    default_tool_callables = {
    ...
    print(f"Warning: Could not import default tools: {e}")
    default_tools = []
    default_tool_callables = {}
...
class OpenAIAgentWithMCP:
    ...
    def __init__(
        ...
        Args:
            ...
            tool_definitions (List[Dict], optional): List of regular tool function definitions.
                Defaults to tools from helper.tools_definition if not provided.
            tool_callables (Dict[str, Callable], optional): Mapping of regular tool names to functions.
                Defaults to callables from helper.tools if not provided.
            system_prompt (str, optional): System message to initialize conversation.
                Defaults to a helpful assistant prompt if not provided.
            ...
            mcp_config_file (str, optional): Path to MCP configuration JSON file.
        ...
        if final_system_prompt:
            self.messages.append({"role": "system", "content": final_system_prompt})
        ...
        # Load MCP configuration from file if not provided
        if mcp_servers_config is None and mcp_config_file is not None:
            mcp_servers_config = self._load_mcp_config(mcp_config_file)
        elif mcp_servers_config is None:
            # Try to load from default location
            mcp_servers_config = self._load_mcp_config("mcp_config.json")

        # Initialize MCP manager if config available
        ...
        self.tool_definitions = tool_definitions or default_tools.copy()
        self.tool_callables = tool_callables or default_tool_callables.copy()
    ...
    def _load_mcp_config(self, config_file: str) -> Dict[str, Dict[str, Any]]:
        """
        Load MCP server configuration from JSON file.
        
        Args:
            config_file: Path to the MCP configuration JSON file
            
        Returns:
            Dictionary of MCP server configurations, or empty dict if file not found
        """
        try:
            ...
            # Extract server configurations, filtering out disabled ones
            servers_config = {}
            if "mcpServers" in config:
                ...
                print(f"Loaded {len(servers_config)} MCP servers from {config_file}")
                return servers_config
            else:
                print(f"No 'mcpServers' section found in {config_file}")
                return {}
                
        except json.JSONDecodeError as e:
            print(f"Error parsing MCP config file {config_file}: {e}")
            return {}
        except Exception as e:
            print(f"Error loading MCP config file {config_file}: {e}")
            return {}
    ...
    def cleanup(self):
        """
        Explicitly cleanup MCP resources and stop all servers.
        Call this method before the script exits to ensure clean shutdown.
        """
        if self.mcp_manager:
            try:
    ...
    def add_mcp_server(
        ...
        if success:
            ...
            new_callables = {
                k: v for k, v in mcp_callables.items() if k.startswith(f"{name}_")
            }
    ...
    def list_available_tools(self) -> Dict[str, List[str]]:
        ...
        for tool_def in self.tool_definitions:
            ...
            if "_" in tool_name and any(
                tool_name.startswith(f"{server}_")
                for server in (
                    self.mcp_manager.client.servers.keys() if self.mcp_manager else []
                )
            ):
        ...
        return {"regular": regular_tools, "mcp": mcp_tools}
